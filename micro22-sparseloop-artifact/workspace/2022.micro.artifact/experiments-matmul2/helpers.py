...
def plot_latency(xml_path):
    stats = parse_timeloop_stats(xml_path)
    latency_cycles = stats.get("cycles", None)

    if latency_cycles is None:
        raise ValueError("Could not find 'cycles' in parsed stats.")

    plt.figure(figsize=(6, 4))
    plt.bar(["Latency"], [latency_cycles], color="firebrick")
    plt.ylabel("Cycles")
    plt.title("Latency from timeloop stats")
    plt.tight_layout()

    output_path = os.path.join("fig_latency.png")
    plt.savefig(output_path)
    print("Figure saved to", os.path.abspath(output_path))
...
def plot_read_write_energy(energy_breakdown_pJ):
    ...
    print(levels)
    levels.remove("BackingStorage")
    read_energies = []
    write_energies = []

    for level in levels:
        stats = energy_breakdown_pJ[level]
#         if level == "BackingStorage":
#             continue
